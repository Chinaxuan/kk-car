...
| 快速进入高频任务 | MaVo 菜单栏入口短；VoHive 用导航分开短信和设置 | 首屏新增电话、短信、信号、套餐四个实时入口，点击在同页定位；详细数据仍完整保留 |
| 短信阅读 | VoHive 的列表与详情分区，MaVo 按需打开详情 | 现有合并短信列表和详情保留，正文只有点击后读取；后续可再做按联系人会话视图 |
| 来电与声音故障 | MaVo / DJOneHubNative 把通话与音频状态独立呈现 | 电话卡片持续显示通话状态；浏览器音频断开时提供手动重连，但不代替真实通话稳定性测试 |
