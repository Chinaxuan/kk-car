...
GNSS 状态命令可用，但外壳天线和定位尚未实测，页面不开放定位按钮；eSIM 管理未获支持证据，也不开放。部署时先放齐 `dji-at-status.sh`、`dji-sms.uc`、`dji-control.sh`、`dji-traffic.uc` 与 `dji-traffic-parse.uc`，再安装 RPC、菜单、ACL 与 `dji-console.js`/`dji-console.css`，只刷新 rpcd 和短信/流量后台服务，不重启 network。设备私有目录只保存流量数字与校正锚点，不保存短信正文。完整边界见公共仓库 `docs/DJI-CONTROL.md`。
