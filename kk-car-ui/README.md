...
硬件保护设置支持 2750–3900 mV，须对照实际电芯规格使用。自动模式允许空电基准与保护电压相等；手动模式要求空电更低，以避开厂家 V9 源码的电压钳位行为。保护寄存器为 0 不能当作关闭保护。软件低电关机仍默认关闭，写入读回不代表实际低压关断和容量校准已验收；固件检查、部署与恢复说明见公开仓库 `docs/UPS.md`。
