...
GNSS 可手动启停并查询位置与速度，但外壳天线及实际定位 fix 尚未验收；只有真实 fix 才显示坐标，默认保持关闭。电话自动只读查询语音通话状态，当前 USB 音频与 IMS 条件不足，拨号/接听不开放。eSIM 管理未获支持证据，也不开放。部署时先放齐 `dji-at-status.sh`、`dji-sms.uc`、`dji-control.sh`、`dji-traffic.uc` 与 `dji-traffic-parse.uc`，再安装 RPC、菜单、ACL 与 `dji-console-v2.js`/`dji-console-v2.css`，只刷新 rpcd 和 LuCI 菜单缓存，不重启 network。设备私有目录只保存流量数字与校正锚点，不保存短信正文。完整边界见公共仓库 `docs/DJI-CONTROL.md`。
