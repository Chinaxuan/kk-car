...
return view.extend({
        ...
        this.controls.replaceChildren(
            ...
            E('div',{'class':'ku-settings-grid'},[
                E('section',{'class':'ku-panel'},[E('h3',{},'电池参数 · 高级'),E('p',{'class':'ku-helper'},'仅在确认电池型号及外部供电稳定时修改。当前原值已显示；改动需输入确认文字。'),
                    ...
                    this.controlRow('电池保护电压','protect_mv',b.configured_protect_mv,0,3900,'0 或 3000–3900 mV；0 为当前原始值',true,false),
                    this.controlRow('用户电池参数','user_programmed',b.user_programmed?1:0,0,1,'启用前需核对三项电压关系',true,true)]),
    ...
    paint:function(d){
        ...
        var b=d.battery||{},i=d.input||{},o=d.output||{},c=d.controller||{},s=d.sensors||{},pi=s.pi_supply||{},bat=s.battery||{},rtc=s.rtc||{};
        ...
        this.metrics.replaceChildren(
            ...
            section('电池与充放电',[cell('UPS 电池端',volts(b.millivolts),'主控读数'),cell('电池侧传感器电压',volts(bat.bus_mv),'INA219 0x45'),cell('电池估算电流',number(bat.current_ma,0)+' mA',bat.current_ma>50?'正值约定为充电':bat.current_ma< -50?'负值约定为放电':'接近平衡'),cell('电池估算功率',number(bat.power_mw/1000,2)+' W','依据 INA219 采样电阻'),cell('电池采样电阻压降',number(bat.shunt_uv/1000,2)+' mV'),cell('电池温度',number(b.temperature_c,0)+' °C'),cell('满电基准',millivolts(b.configured_full_mv)),cell('空电基准',millivolts(b.configured_empty_mv)),cell('保护电压',millivolts(b.configured_protect_mv))],'估算电流/功率采用厂商标注电阻值，尚未外部校准'),
